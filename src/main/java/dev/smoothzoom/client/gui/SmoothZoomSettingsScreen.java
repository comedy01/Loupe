package dev.smoothzoom.client.gui;
...
public final class SmoothZoomSettingsScreen extends OptionsSubScreen {
    private static final int WIDE_BUTTON = 310;

    public SmoothZoomSettingsScreen(Screen lastScreen, Options options) {
        super(lastScreen, options, Component.translatable("smoothzoom.options.title"));
    }
    ...
    protected void addOptions() {
        ...
        list.addBig(slider(
                "smoothzoom.options.amount",
                "smoothzoom.options.amount.tooltip",
                ZoomMath.MIN_AMOUNT,
                ZoomMath.MAX_AMOUNT,
                0.25,
                config.zoomAmount(),
        ...
        list.addBig(slider(
                "smoothzoom.options.speed",
                "smoothzoom.options.speed.tooltip",
                ZoomMath.MIN_SPEED,
                ZoomMath.MAX_SPEED,
                0.5,
                config.zoomSpeed(),
        ...
        list.addBig(slider(
                "smoothzoom.options.look",
                "smoothzoom.options.look.tooltip",
        ...
        list.addBig(OptionInstance.createBoolean(
                "smoothzoom.options.scroll",
                OptionInstance.cachedConstantTooltip(Component.translatable("smoothzoom.options.scroll.tooltip")),
                config.scrollToZoom(),
                config::setScrollToZoom));

        KeyMapping key = SmoothZoomClient.zoomKey();
        Component keyLabel = key == null
                ? Component.translatable("smoothzoom.options.key.unavailable")
                : Component.translatable("smoothzoom.options.key", key.getTranslatedKeyMessage());
        ...
        list.addBig(Button.builder(
                        Component.translatable("smoothzoom.options.reset"),
                        button -> {
